...
// Robosense LiDAR point structure
...
private:
 ...
 int message_processed_ = 0; 
    ...
    bool PointCloud2_to_PointXYZ_unwrap_robosense(pcl::PointCloud<robosenseM1_ros::Point> &in, std::vector<pcl::PointXYZ> &out, double scan_end_time);
...
void DLO3DNode::pointcloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& cloud, const sensor_msgs::msg::PointCloud2::ConstSharedPtr& cloud_aux, bool is_aux_avaliable)
{
    ...
    if (!printed_fields) {
        ...
        RCLCPP_INFO(this->get_logger(), "LiDAR Type: %s", m_lidarType.c_str());
    }

    double scan_end_time = cloud->header.stamp.sec + cloud->header.stamp.nanosec * 1e-9;
    std::vector<pcl::PointXYZ> c;
    bool unwrap_success = false;

    // Process based on LiDAR type
    if (m_lidarType == "robosense") {
        // Robosense LiDAR processing
        ...
        pcl::PointCloud<robosenseM1_ros::Point> transformed_cloud_rs;
        pcl::transformPointCloud(pcl_cloud_rs, transformed_cloud_rs, transform_matrix);
        pcl::PointCloud<robosenseM1_ros::Point> final_cloud_rs = transformed_cloud_rs;

        // Fuse auxiliary cloud if available (for Robosense)
        if (is_aux_avaliable) {
            Eigen::Matrix4f transform_aux_matrix = getTransformMatrix(m_staticTfPointCloudAux);
            pcl::PointCloud<robosenseM1_ros::Point> pcl_cloud_aux_rs;
            pcl::fromROSMsg(*cloud_aux, pcl_cloud_aux_rs);

            pcl::PointCloud<robosenseM1_ros::Point> transformed_cloud_aux_rs;
            pcl::transformPointCloud(pcl_cloud_aux_rs, transformed_cloud_aux_rs, transform_aux_matrix);
            final_cloud_rs += transformed_cloud_aux_rs;
        }

        unwrap_success = PointCloud2_to_PointXYZ_unwrap_robosense(final_cloud_rs, c, scan_end_time);

        // If unwrap not succeeded, use raw cloud
        if (!unwrap_success) {
            int points = 0;
            double down_size = 0;
            c.clear();
            c.reserve(final_cloud_rs.size());

            for (const auto& point : final_cloud_rs) {
        ...
        // Ouster/Default LiDAR processing
        ...
        // Fuse auxiliary cloud if available
        if (is_aux_avaliable) {
            Eigen::Matrix4f transform_aux_matrix = getTransformMatrix(m_staticTfPointCloudAux);
        ...
        unwrap_success = PointCloud2_to_PointXYZ_unwrap(final_cloud, c, scan_end_time);
...
//! Robosense LiDAR point cloud unwrapping function
    ...
    double scan_end_time)
{
    ...
    // Find the Filter transform corresponding to the time of the scan end
    Closest_Filter_Result Filter_f = findClosestFilterData(filtered_deque, scan_end_time);
    ...
    for (const auto& pt : in.points)
    {
        ...
        // Robosense uses absolute timestamp in double (seconds)
